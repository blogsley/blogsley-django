...
def extract_active_tags(html: str) -> list:
    ...
    for p in tag_positions:
        if html[p] == "<":
            ...
            if html[p : p + 4] == "<!--":
                if in_comment:
                    logger.debug("nested comments are not allowed")
                    raise HtmlCommentError("nested comments are not allowed")
                in_comment = True
                continue
            if in_tag:
                logger.debug("nested tag found")
            ...
            if p > 1 and html[p - 2 : p + 1] == "-->":
                if not in_comment:
                    logger.debug("cannot close comment when no comment opened")
            ...
            if in_comment:
                continue
            if not in_tag:
                logger.debug("cannot close tag that was not opened")
    ...
    # entire list
    if in_tag:
        logger.debug("file cannot end with unclosed tag")
        raise InvalidHtmlTagError("file cannot end with unclosed tag")
    if in_comment:
        logger.debug("file cannot end with an open comment tag")
